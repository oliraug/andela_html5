...
class webcamClass extends connectionClass{
    ...
    public function updateImageToDatabase($imageurl){
        $image=$imageurl;
        $updateid = sqlInsert("UPDATE snapshot SET Image = $Image WHERE pid = $pid", array($image, $_SESSION['pid']));
         $result= $this->query($updateid);
         return $result;
    }
    ...
    public function saveImageToDatabase($imageurl){
      ...
      $id = 0 + (isset($_GET['pid']) ? $_GET['pid'] : '');
        ...
        if ($id && $id != 0) {
                updateImageToDatabase($imageurl);
        } else {
